        ...
        return  res.status(400).send({
          ...
          Media.find(function(err, docs){
            docs.forEach(function(doc){
                ig.likes(doc.media_id, function(err, result,cb) {
                    if(err){
                        console.log(err);
                    }
                    let usersLike = {
                    ...
                    //     console.log(result,"resutlt only have value")
                    //     usersLike.push(result);
                    //     // console.log(usersLike,"media id", doc.media_id);
                    //     saveLike(usersLike);
                    // }else{
                    //     next();
                    // }
...
let saveLike = function(data) {
    MediaLike(data).save(data , function(err, result) {
        if (err) {
            return err;
        }else {
